/**
 * Buscojobs Job Scraper - Bolsa de trabajo para El Salvador
...
const BASE_URL = 'https://www.buscojobs.com.sv';
const MAX_RETRIES = 3;
        ...
        try {
            ...
            if (res.status === 403 || res.status === 429) {
                if (attempt < retries) {
                    ...
                    console.log(`[Buscojobs] Bloqueo ${res.status}. Reintentando en ${waitTime}ms...`);
            ...
            const waitTime = BASE_DELAY * Math.pow(2, attempt);
            console.log(`[Buscojobs] Error: ${err.message}. Reintentando en ${waitTime}ms...`);
            await delay(waitTime);
        }
    }
}
...
/**
 * Realiza scraping de Buscojobs El Salvador
 * Es una bolsa de trabajo local con opciones de tecnología y desarrollo
 */
async function scrapeBuscojobs(searchQuery = 'desarrollador') {
    console.log(`[Buscojobs] Iniciando scraping para: "${searchQuery}"`);
    
    const jobs = [];
    
    // Buscojobs tiene una API de búsqueda
    const searchUrl = `${BASE_URL}/api/search?q=${encodeURIComponent(searchQuery)}&country=SV`;
    ...
    try {
        ...
        console.log(`[Buscojobs] Error en API principal: ${err.message}`);
    }
    
    // Método alternativo:爬取页面
    if (jobs.length === 0) {
        jobs.push(...await tryAlternative(searchQuery));
    }
    
    console.log(`[Buscojobs] Scraping completado: ${jobs.length} ofertas`);
    return jobs;
}
...
function parseJob(job) {
    return {
        ...
        url: job.url || job.link || `${BASE_URL}/empleo/${job.id}`,
...
async function tryAlternative(searchQuery) {
    ...
    // Acceder a la búsqueda pública de Buscojobs
    const searchPage = `${BASE_URL}/buscar?q=${encodeURIComponent(searchQuery)}`;
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Buscojobs estructura sus ofertas con article o div con clase job
            const jobMatches = html.matchAll(/<article[^>]*class="[^"]*job[^"]*"[^>]*>[\s\S]*?<a[^>]*href="([^"]+)"[^>]*>[\s\S]*?<h[23][^>]*>([^<]+)<\/h[23]>[\s\S]*?class="company[^"]*"[^>]*>([^<]+)<\/div>/gi);
        ...
        console.log(`[Buscojobs] Método alternativo falló: ${err.message}`);
    }
    ...
    for (const job of jobs) {
        try {
            ...
            if (response.status === 409) {
                ...
                newCount++;
                console.log(`[Buscojobs] Nuevo: "${job.title}" en ${job.company}`);
            }
        } catch (err) {
            console.log(`[Buscojobs] Error: ${err.message}`);
        }
        
        await delay(600);
    }
    
    console.log(`[Buscojobs] Resumen: ${newCount} nuevos, ${duplicateCount} duplicados`);
...
async function main() {
    console.log('[Buscojobs] === Scraper iniciado ===');
    
    const searchQuery = process.argv[2] || 'desarrollador';
    
    try {
        const jobs = await scrapeBuscojobs(searchQuery);
        if (jobs.length > 0) {
            await postJobs(jobs);
        }
    } catch (err) {
        console.error('[Buscojobs] Error fatal:', err.message);
    }
    
    console.log('[Buscojobs] === Scraper finalizado ===');
}

module.exports = { scrapeBuscojobs };
